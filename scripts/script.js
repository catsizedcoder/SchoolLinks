...
function checkValues() {
  ...
  if (document.getElementById("series").value.trim() == defaultParams.series) {
...
function syncQueryToSearch() {
  if (searchQueryObject.search) {
    document.getElementById("search-bar").value = searchQueryObject["search"];
  }
  ...
  if (searchQueryObject.include) {
    document
      .querySelectorAll('#search-filters input[type="checkbox"]')
      .forEach(function (input) {
        input.checked = searchQueryObject["include"].includes(input.value);
      });
  }
